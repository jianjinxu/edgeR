...
y[1:10,]
d<-new("DGEList",list(data=y,group=rep(1:2,each=2),lib.size=lib.sizes))
d
names(d)
@
...
k<-(adj.p<.05)
plot( (log2(ms$ps$p1)+log2(ms$ps$p2))/2, log2(ms$ps$p2/ms$ps$p1), pch=19, xlab="A",ylab="M",col=c("black","blue")[k+1])
@
