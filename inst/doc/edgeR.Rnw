...
The object produced by \texttt{exactTest} contains three elements:
\texttt{table}, \texttt{comparison} and \texttt{genes}. The element \texttt{de.com\$comparison} contains a vector giving the names of the two groups compared. The table\texttt{de.com\$table} contains the elements \texttt{logConc}, which gives the overall concentration for a tag across the two groups being compared, \texttt{logFC}, which gives the log-fold change difference for the counts between the groups and \texttt{p.value} gives the exact $p$-values computed.
...
In Figure~\ref{fig:Zhang_FC1}, the $264$ tags identified as differentially expressed (i.e.~those identified as significant ($p$-value less than $0.01$) by \texttt{edgeR} using the common dispersion) are outlined in red.

\begin{figure}[ht]
\begin{center}
\includegraphics[width=\textwidth]{edgeR_case_study_Zhang-015.png}
...
parameter \texttt{prior.n}. The value for \texttt{prior.n} corresponds
to the number of individual tags equivalent to the weight given to the
common likelihood. Thus, the higher \texttt{prior.n}, the more
...
question, but we currently recommend selecting a value for the weight parameter \texttt{prior.n} \emph{a
...
for \texttt{prior.n} such that individual tagwise dispersion estimates are `squeezed' quite strongly
towards the common dispersion. Here, we choose a moderate amount of
smoothing---we let \texttt{prior.n} be $10$. This means that the
...
estimate from \texttt{estimateCommonDisp}. The smoothing parameter,
\texttt{prior.n} can then be calculated. As we see below, for this dataset the estimate of the smoothing parameter is very large. This estimate arises from the fact that the estimate of the overdispersion
...
\texttt{prior.n}. If we were to use this large value for the weight parameter we would moderate the tagwise dispersion estimates so strongly that it would be equivalent to using the common dispersion estimate.
...
but we add the argument \texttt{common.disp=FALSE} to the call to
\texttt{exactTest}. Here we
carry out the testing using the tagwise dispersion estimates
calculated using a \texttt{prior.n} value of ten.
...
\texttt{edgeR} package still identifies a lot of differential
...
\texttt{prior.n}, the more highly variable tags will be penalised, as
...
We see in the output below that $309$ genes are significantly differentially expressed according to \texttt{edgeR} when using the tagwise dispersion estimates (ten fewer than when using the common dispersion). Of those tags, $100$ are up-regulated in the cancer cells compared with the normal cells and $209$ are down-regulated in the cancer cells compared with normal cells. These proportions of up- and down-regulated tags are similar to those found using the common dispersion, but there is a slightly higher proportion of down-regulated tags in those identified as DE using tagwise dispersions.
...
just be demonstrating the use of \texttt{edgeR}. 

\begin{figure}[ht]
\begin{center}
\includegraphics[width=0.9\textwidth]{edgeR_case_study_longSAGE_MDSplot.pdf}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=\textwidth]{edgeR_case_study_longSAGE-015.png}
...
value for \texttt{prior.n} \emph{a priori} that will provide an
...
\subsubsection{Estimating the smoothing parameter using an approximate
  eBayes rule}
Although we recommend using a fixed value for the \texttt{prior.n}
...
method is implemented in \edgeR. The smoothing parameter, \texttt{prior.n} can be calculated using
the function \texttt{estimateSmoothing}, which takes the
\texttt{DGEList} object produced by \texttt{estimateCommonDisp} as its
...
determining differential expression using \texttt{exactTest}. Here we
carry out the testing using the tagwise dispersion estimates
calculated using a \texttt{prior.n} value of ten.
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=0.9\textwidth]{edgeR_case_study_Li_MDSplot.pdf}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=\textwidth]{edgeR_case_study_Li-017.png}
\caption{Plot of the log-fold change against the log-concentration for each tag. The $500$ most differentially expressed tags as identified by \texttt{edgeR} using the common dispersion are outlined in red.}
\label{fig:Li_FC1}
\end{center}
\end{figure}
...
it is recommended to use a larger value for \texttt{prior.n}, which
...
setting the \texttt{prior.n} to be ten (as we have done previously)
...
estimates. We currently recommend that the \texttt{prior.n} value
given by \texttt{estimateSmoothing} be used as a guide, but not
routinely in analysis.

The function \texttt{estimateSmoothing} estimates the smoothing
parameter \texttt{prior.n} and takes the \texttt{DGEList} object
produced by \texttt{estimateCommonDisp} as its argument. As we see
...
selecting a larger value for \texttt{prior.n}, as outlined above.
