...
To illustrate this, we sample Poisson data and run \texttt{deDGE} with the \texttt{doPoisson} argument set to \texttt{TRUE}.  The data is quantile-adjusted and the exact test is invoked after first setting the dispersion parameter to 0.  To observe that it does a normalization of sorts, we can again look at boxplots.

<<poisson,fig=TRUE,width=7,height=4>>=
set.seed(101)
n <- 100
mu <- 10
lib.sizes <- c(10,8,14,10)*1000
