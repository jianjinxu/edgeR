...
# Created 2009. Last modified 3 Oct 2011.
{
	...
	if(method=="grid") {  # do a grid search, since some likelihoods may be monotone, not amenable to NR
		...
		m0 <- switch(trend,
			...
			"tricube" = ntags*weightedComLik(object,l0,prop.used=prop.used), # Weights sum to 1, so need to multiply by number of tags to give this the same weight overall as the regular common likelihood
