...
#	Do two iterations of calculating pseudodata and estimating common dispersion, first one uses Poisson
{
	...
	levs.group <- levels(group)
		...
		d <- new("DGEList",list(samples=object$samples, common.dispersion=1e-16, counts=object$counts, 
                                  pseudo.alt=q2q.out$pseudo, conc=q2q.out$conc, common.lib.size=q2q.out$N))
                return(d)
        }
	for(i in seq_len(2)) {
	...
	new("DGEList",list(samples=object$samples, common.dispersion=common.dispersion$dispersion, 
                           counts=object$counts, pseudo.alt=q2q.out$pseudo, conc=q2q.out$conc, common.lib.size=q2q.out$N))
}
